...
def train_conv_net(train,
    ...
    zero_vec_tensor = T.vector(dtype = theano.config.floatX)
    zero_vec_pf = np.zeros(pf_dim).astype(theano.config.floatX)
...
if __name__ == "__main__":
    ...
    print 'load Wv ...'
    Wv = cPickle.load(open(inputdir+'/'+str(dimension)+'/Wv.p')).astype(theano.config.floatX)

    rng = np.random.RandomState(3435)
    PF1 = np.asarray(rng.uniform(low=-1, high=1, size=[101, 5]))
    padPF1 = np.zeros((1, 5))
    PF1 = np.vstack((padPF1, PF1)).astype(theano.config.floatX)
    PF2 = np.asarray(rng.uniform(low=-1, high=1, size=[101, 5]))
    padPF2 = np.zeros((1, 5))
    PF2 = np.vstack((padPF2, PF2)).astype(theano.config.floatX)
